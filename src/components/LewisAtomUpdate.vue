<template>
  <g>
    ...
    <lewis-atom-update v-for="atom in toDraw.newAtoms" :stats="atom.stats" :bboxes="bboxes" :key="atom.stats.index"></lewis-atom-update>
  </g>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    placedAtoms: function() {
      if(this.stats.placedAtoms.reduce((a,b) => a + b, 0) == 0){
        //no atoms placed yet, so let's set up the backbone
        //might also want to setup perif posiitons from here to make sure they don't overlap
        let placedArray = this.stats.placedAtoms.slice(0)
        let structure = this.stats.atomsArray
        let backboneArray = []
        let loopArray = []

        function isBackboneAtom(structure, index) {
          let atom = structure[index]
          if (atom[3].length > 1) return true
          else return false
        }

        //fill backbone array with [index of atom in structure, backbone indices its connected to]
        structure.forEach((element, index, structure) => {
          if(isBackboneAtom(structure, index)) {
            array_i = [element[0]]
            element[3].forEach(element => {
              if (isBackboneAtom(structure, element[0])){
                array_i.push(element[0])
              }
            })
            backboneArray.push(array_i)
            loopArray.push(array_i)
          }
        })
        console.log(backboneArray)

        //identify loops
        let prevLength = loopArray.length
        let currLength = loopArray.length
        let maybeLoop = []
        do {
          loopArray = loopArray.filter(element => element.length > 2)
          loopArray.forEach((element) => {
            maybeLoop.push(element[0])
          })
          loopArray.forEach(element => {
            element.filter(el => maybeLoop.includes(el))
          })
          prevLength = currLength
          currLength = loopArray.length
        } while(currLength != prevLength)

        if (loopArray.length > 0) {

          //label loop atoms in structure??
          loopArray.forEach(element => {
            structure[element[0]][4] = 1
          })

          //check for fused loops (any element of loopArray with length > 3)
        ...
        let centerArray = []
        centerArray[this.index] = this.stats.center

            

        //need a good way to specify what the places of the atoms will be, maybe by center?
        //maybe call toDraw from here? maybe break toDraw into smaller functions?

        return placedArray
      }
      else {
        //get placedArray and centerArray passed as props?
      }
    },
    textRect: function() {
